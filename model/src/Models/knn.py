...
def generate_fmea_diagnosis_knn(analyses):
    causes = []
    recommendations = []
...
def train_and_detect_anomalies_from_df(df: pd.DataFrame):
    try:
        ...
        df['Anomaly'] = (avg_distances > threshold).astype(int) * -1  # -1 for anomaly, 0 for normal
        ...
        for idx, row in df[df['Anomaly'] == -1].iterrows():
            ...
            # FMEA Diagnosis
            diagnosis = generate_fmea_diagnosis_knn(analyses)
            alert_level = max(
